...
async function saveEdit(session: StudySession) {
  ...
  try {
    ...
    saveErrors[session.id] = extractErrorMessage(error)
  } finally {
    savingId.value = null
  }
}
          ...
          <div class="flex flex-wrap items-end gap-2">
            <label :for="`edit-session-${session.id}`" class="flex flex-col gap-1 text-xs font-medium text-slate-600">
              ...
              <input
                ...
                class="w-32 rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm disabled:bg-slate-100 disabled:text-slate-400"
              >
            </label>
            <button
              type="button"
              :disabled="savingId === session.id"
              class="rounded-lg bg-indigo-600 px-3 py-2 text-sm font-semibold text-white disabled:cursor-not-allowed disabled:opacity-60"
            ...
            <button
              ...
              @click="cancelEdit(session.id)"
            >
              Cancel
            </button>
          </div>
          <p v-if="saveErrors[session.id]" class="text-sm text-red-700" role="alert">
            {{ saveErrors[session.id] }}
          </p>
        </div>
        <div v-else class="flex flex-col gap-2">
          <div class="flex flex-wrap items-center justify-between gap-3 text-sm">
            <div>
              ...
              <span class="ml-2 text-xs text-slate-400">{{ formatRecordedAt(session.createdAt) }}</span>
            </div>
