...
function extractErrorMessage(error: unknown): string {
  ...
  return (
    ...
    "Could not update the subject. Please try again."
  );
}
...
async function handleSubmit() {
  ...
  try {
    const response = await $fetch<{ status: string; subject: Subject }>(
      `/api/subjects/${props.subject.id}`,
      {
        method: "PATCH",
        body: {
          name: form.name.trim(),
...
<template>
  <form novalidate class="flex flex-col gap-3" @submit.prevent="handleSubmit">
    <div class="flex flex-col gap-1">
      <label
        :for="`subject-edit-name-${subject.id}`"
    ...
    <div class="flex flex-col gap-1">
      <label
        :for="`subject-edit-description-${subject.id}`"
        class="text-sm font-medium text-slate-700"
