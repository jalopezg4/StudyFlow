...
describe('CreateStudyTaskSchema', () => {
  it('accepts a valid subjectId and title with no other fields', () => {
    const result = CreateStudyTaskSchema.safeParse({ subjectId: 'subject-1', title: 'Read chapter 3' })

    expect(result.success).toBe(true)
    if (result.success) {
      expect(result.data).toEqual({ subjectId: 'subject-1', title: 'Read chapter 3' })
    }
  })
  ...
  it('rejects a missing title', () => {
    const result = CreateStudyTaskSchema.safeParse({ subjectId: 'subject-1' })

    expect(result.success).toBe(false)
  })

  it('rejects an empty title', () => {
    const result = CreateStudyTaskSchema.safeParse({ subjectId: 'subject-1', title: '' })
  ...
  it('accepts a title at exactly the 100 character limit', () => {
    const result = CreateStudyTaskSchema.safeParse({ subjectId: 'subject-1', title: 'a'.repeat(100) })

    expect(result.success).toBe(true)
  })

  it('rejects a title over the 100 character limit', () => {
    const result = CreateStudyTaskSchema.safeParse({ subjectId: 'subject-1', title: 'a'.repeat(101) })
